...
class SigmaFileSystem:
    ...
    def move_file(self, fileId: int, newFolderId: int) -> None:
        if fileId not in self.all_files:
            ...
            return
        folderId, fileName = self.all_files[fileId]
        ...
        if folderId in self.dashboard:
            ...
            self.dashboard[newFolderId][fileName] = int("1100"+ str(len(self.dashboard)))
        elif folderId in self.worksheet:
            del self.worksheet[folderId][fileName]
            self.worksheet[newFolderId][fileName] = int("1010"+ str(len(self.worksheet)))
        elif folderId in self.folder:
            del self.folder[folderId][fileName]
            self.folder[newFolderId][fileName] = int("1001"+ str(len(self.folder)))
    ...
    def print_files(self) -> None:
      # TODO: implement
        res = {"MyDocuments":{"Folders":[],"Documents":{"Dashboards":[],"Worksheets":[]}}}
        for folder in self.dashboard:
            res["MyDocuments"]["Documents"]["Dashboards"].extend(self.dashboard[folder].keys())
        for folder in self.worksheet:
            res["MyDocuments"]["Documents"]["Worksheets"].extend(self.worksheet[folder].keys())
